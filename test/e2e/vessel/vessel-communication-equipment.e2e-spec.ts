...
describe('VesselClassController (e2e)', () => {
  ...
  const mockVesselCommunicationEquipment = {
    typeId: 'e2e-test Comm Equip',
    numberOfUnits: 1,
    vesselId: 1,
  };
  ...
  it('/vessel-communication-equipment-type (POST)', async () => {
    return await request(app.getHttpServer())
      ...
      .send(mockVesselCommunicationEquipment)
      .expect(HttpStatus.CREATED)
      .expect((res) => {
        id = res.body.id;
        expect(res.body.typeId).toEqual(
          mockVesselCommunicationEquipment.typeId,
        );
        expect(res.body.numberOfUnits).toEqual(
          mockVesselCommunicationEquipment.numberOfUnits,
        );
      });
  });

  xit(`/vessel-communication-equipment-type?name=${mockVesselCommunicationEquipment.name} (GET)`, async () => {
    return await request(app.getHttpServer())
      .get(
